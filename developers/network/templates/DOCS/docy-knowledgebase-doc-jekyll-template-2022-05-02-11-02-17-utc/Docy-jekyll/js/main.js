(function ($) {
  ...
  if (
    $(".blog_single2_left_sidebarlist .nav-sidebar > li").hasClass("active")
  ) {
    $(".blog_single2_left_sidebarlist .nav-sidebar > li.active")
      .find("ul")
      .slideDown(700);
  }

  function active_dropdown3() {
    $(".blog_single2_left_sidebarlist .nav-sidebar > li > a").on(
      "click",
      function (e) {
        event.preventDefault();
        $(this).parent().find("ul").first().toggle(300);
  ...
  $(".blog_single2_left_sidebarlist .nav-sidebar > li > a").each(function () {
    var $this = $(this);
    $this.on("click", function (e) {
      event.preventDefault();
      var has = $this.parent().hasClass("active");
  ...
  if ($(".rvfs-section").length > 0) {
    $.rvFontsize({
      targetSection: ".rvfs-section",
  ...
  $('.header_search_form,.banner_search_form').on('submit',function (e) {
    e.preventDefault();
 $('#search-hero-results li:first-child').find(".search_item li:first-child a")[0].click();
  })
  ...
  function bodyFixed2() {
    var windowWidth = $(window).width();
    var bottom = $(window).height() - $(".blog_single2_info").height() - 50;
    if ($("#sticky_doc2").length) {
      if (windowWidth > 576) {
        var tops = $("#sticky_doc2");
        var leftOffset = tops.offset().top;
        var content = $(".blog-single-2-comment-form");
        var contentTop = content.offset().top - 300;

        $(window).on("scroll", function () {
          var scroll = $(window).scrollTop();
          if (scroll >= leftOffset && scroll <= contentTop) {
            tops.addClass("stick");
          } else {
